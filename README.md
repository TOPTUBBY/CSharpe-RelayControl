...
**Arduino IDE / ESP32-C3 จริง:** เปิดไฟล์ `firmware/ESP32C3_RelayControl/ESP32C3_RelayControl.ino` ใน Arduino IDE โดยตรง (อย่าใช้ไฟล์ ESP32 รุ่นปกติหรือสำเนาเก่าจาก Wokwi) ตั้ง `RELAY_ACTIVE_HIGH` เป็น `0` ตามค่าเริ่มต้น เลือกบอร์ด ESP32-C3 และพอร์ตของบอร์ดจริง แล้วกด Upload; หากใช้ USB ภายในให้เปิด USB CDC On Boot ตามการตั้งค่าบอร์ด `driver/gpio.h` มากับ ESP32 Arduino core และไม่ต้องติดตั้งไลบรารีแยก หลังอัปโหลดให้ทดลอง ON/OFF ทีละช่องกับโมดูลจริงและตรวจ `COM–NO` ด้วย
